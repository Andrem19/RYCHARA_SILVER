...
def place_market_order(settings: Settings, sd: str):
    if sv.settings_gl.exchange =='BB':
        BB.open_order('market', settings.coin, sd, settings.amount_usdt, False)
    elif sv.settings_gl.exchange == 'KC':
        KuCoin.open_market_order(settings.coin, sd, settings.amount_usdt)
    elif sv.settings_gl.exchange == 'OK':
        OKX.open_order('market', settings.coin, sd, settings.amount_usdt, False)
    elif sv.settings_gl.exchange == 'BG':
        BG.open_order('market', settings.coin, sd, settings.amount_usdt, False)
    elif sv.settings_gl.exchange == 'BX':
        BX.open_order('market', settings.coin, sd, settings.amount_usdt, False)
    elif sv.settings_gl.exchange == 'BM':
        BM.open_order('market', settings.coin, sd, settings.amount_usdt, False)
    elif sv.settings_gl.exchange == 'GT':
        GT.open_order(settings.coin, sd, settings.amount_usdt, False)
    elif sv.settings_gl.exchange == 'BN':
        BN.open_order('market', settings.coin, sd, settings.amount_usdt, False)
...
async def place_universal_order(settings: Settings, buy_sell: int):
    ...
    open_time = datetime.datetime.now().strftime(sv.time_format)
    print('order placed')
    while True:
